...
public class JumperSkill : MonoBehaviour
{
    [Header("References")]
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0) && canPlatform == true)
        {
            ...
            {
                platformClone = Instantiate(platform, hit.point, tr.transform.rotation * platform.transform.rotation);
            }
            else
            {
                platformClone = Instantiate(platform, tr.position + transform.forward * 10, tr.transform.rotation * platform.transform.rotation);
            }
        ...
        if (Physics.Raycast(tr.position, tr.TransformDirection(Vector3.forward), out hit, 10))
        {
            ...
            platformPreview.transform.rotation = tr.transform.rotation * platform.transform.rotation;
        }
        else
        {
